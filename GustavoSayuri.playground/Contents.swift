...
class MeuBtn : UIButton{
    ...
    private func desenharBtn(){
        ...
        layer.borderWidth = 2
        layer.borderColor = #colorLiteral(red: 0.7843137255, green: 0.8470588235, blue: 0.4117647059, alpha: 1)
    }
...
class ViewController : UIViewController //Controller -> Controla a view
{
    ...
    let perguntas = ["SayQuiz \n Prove que conhece a Sayuri como ninguém ;)", "O nome completo dela é Maria Luiza Harumi Hioki", "Ela nasceu em 20/02/2000", "Seu primeiro curso foi Sistemas de Informação", "Quando criança queria ser bailarina", "Ela dança há 14 anos", "Durante o ensino médio ganhou dois Óscares Acadêmicos de matemática", "Sua banda favorita é Panic! At The Disco", "Antes de vir para São Paulo morava em Osasco", "Parabéns! Você conseguiu chegar até o fim, isso mostra que você conhece muito bem a Sayuri!"]
    ...
    override func loadView() { //Sobrescreve método loadView() -> Dispara quando a view é carregada
        ...
        label.font = UIFont(name: "Baskerville", size: 30)
        ...
        btnV.titleLabel?.font = UIFont(name: "Baskerville", size: 30)
        btnV.tag = 1
        ...
        btnM.titleLabel?.font = UIFont(name: "Baskerville", size: 30)
        btnM.tag = 0
        ...
        //config botao de Inicio
        btnFinal = MeuBtn()
        btnFinal.setTitle("Tentar Novamente", for: .normal)
        btnFinal.titleLabel?.lineBreakMode = NSLineBreakMode.byWordWrapping
        btnFinal.titleLabel?.numberOfLines = 2
        btnFinal.titleLabel?.textAlignment = .center
        btnFinal.titleLabel?.font = UIFont(name: "Baskerville", size: 30)
        ...
        NSLayoutConstraint.activate([    //Funçao que aparentemente define o local dos objetos na tela
            ...
            btnFinal.heightAnchor.constraint(equalToConstant: 150),
            btnFinal.widthAnchor.constraint(equalToConstant: 150),
